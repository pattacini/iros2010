...
bool ReachingThread::init(Searchable &s){
  ...
  if(struc.isNull()){
    printf("no robot structure file given\n");
    return false;
  }
  ...
  if(opt.isNull()){
    ...
    ((KChainBodySchema *)arm)->SetTolerance(0.1);
    cout<<"orientation is discarded"<<endl;
  }
  else{
    cartesian_dim =6;
    arm = new KChainOrientationBodySchema(telesc);
    ((KChainOrientationBodySchema *)arm)->SetTolerance(0.1);
...
//     }
  ...
  float alpha = (float)s.check("alpha",Value(0.01)).asDouble();
  float beta  = (float)s.check("beta",Value(0.005)).asDouble();

  reach = new Reaching(alpha,beta); 
 ...
 ///////////////////////////////////////////////////////////////////////
  ...
  sprintf(tmp2,"/icub/vc/%s/command",partName.c_str());//was fastCommand
    ...
    printf("Cannot connect port %s to port %s\n",tmp1,tmp2);
    return false;
  }
#endif    
  ...
  H0=yarp::math::eye(4,4);

  yarp::sig::Matrix H01(4,4);
  H01(0,0)= 0.0; H01(0,1)= 0.0; H01(0,2)=-1.0; H01(0,3)=22.569;
  H01(1,0)=-1.0; H01(1,1)= 0.0; H01(1,2)= 0.0; H01(1,3)=6.047;
  H01(2,0)= 0.0; H01(2,1)= 1.0; H01(2,2)= 0.0; H01(2,3)=175.300;
  H01(3,0)= 0.0; H01(3,1)= 0.0; H01(3,2)= 0.0; H01(3,3)=1.0;
  H10=iCub::ctrl::SE3inv(H01);

  A=H01;
  A(0,3)=A(1,3)=A(2,3)=A(3,3)=0.0;
  invA=iCub::ctrl::SE3inv(A);

  yarp::sig::Matrix B(4,4);
  B(0,0)= 0.0; B(0,1)= 1.0; B(0,2)= 0.0; B(0,3)=0.0;
  B(1,0)= 0.0; B(1,1)= 0.0; B(1,2)=-1.0; B(1,3)=0.0;
  B(2,0)=-1.0; B(2,1)= 0.0; B(2,2)= 0.0; B(2,3)=0.0;
  B(3,0)= 0.0; B(3,1)= 0.0; B(3,2)= 0.0; B(3,3)=1.0;
  invB=iCub::ctrl::SE3inv(B);
...
bool ReachingThread::matchBodySchemaToRobot(){
    ...
    if(Network::connect("/icub/right_arm/state:o",
      ...
      printf("cannot read robot state\n");;    
    }
    return false;
}
...
bool ReachingThread::initMapping(Searchable& s){
  ...
  if(mapping.isNull()){
    ...
    if(ms != n){
      printf("warning: mapping size %d is not equal to joint size %d",ms,n);
    }
    jointMapping = new int[mappingSize];  
...
bool ReachingThread::listen(){
  ...
  {
      if (cartesian_dim == 3)
          new_t = targetPort.ReadPosition(pos.data());
      else 
          new_t = targetPort.ReadPositionAndOrientation(pos.data(),orien.data());
    ...
    if (new_t)
    {
        ...
        yarp::sig::Matrix H1=H10*H0;        

        float *pPos=tar.GetArray();
        pPos[0]=H1(0,3);
        pPos[1]=H1(1,3);
        pPos[2]=H1(2,3);
        ...
        if (cartesian_dim!=3)
        {           
           yarp::sig::Vector v=iCub::ctrl::dcm2axis(invA*iCub::ctrl::axis2dcm(orien)*invB*A);

           // conversion from axis/angle to Rodriguez notation
           float _sin=sin(v[3]/2.0);
           float *pOrien=tar.GetArray()+3;
           pOrien[0]=_sin*v[0];
           pOrien[1]=_sin*v[1];
           pOrien[2]=_sin*v[2];
        }
        ...
        cout<<endl;
        cout<<"----------"<<endl;
        cout<<"target: ";tar.Print();
        cout<<"achieved target: ";tc.Print();
        cout<<"target joint configuration: ";ta.Print();
        cout<<"----------"<<endl;
        cout<<endl;
        */
    }
